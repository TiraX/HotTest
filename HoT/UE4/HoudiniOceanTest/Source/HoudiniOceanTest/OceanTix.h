...
namespace drw
{
	...
	typedef TiComplex					complex_f;
	typedef std::vector<my_float>		vector_f;
	typedef TiMatrix<my_float>			matrix_f;
	typedef TiMatrix<TiComplex>			matrix_c;
	...
	class OceanContext
	{
	 public:
	  ...
	  void alloc_chop()
	  {
		...
		_disp_x_plan = FFT2D(_M,
			_disp_x.data());

		_disp_z_plan = FFT2D(_M,
			_disp_z.data());
	  }
	  void alloc_normal()
	  {
				  ...
				  float chop_amount)
	  {
		...
		for (int i = 0 ; i  < _M ; ++i)
		{
...
#endif
		  {
			...
			r._htilda(i, j) = r._htilda(i, j) * scale;
		  }
		}
		...
		if (do_heightfield && r._do_disp_y)
		{
		  ...
		  //fftwf_execute(r._disp_y_plan);
			r._disp_y_plan.DoFFT(r._htilda);
		}
		...
		if (do_chop && r._do_chop)
		{
		  ...
		  r._disp_x_plan.DoFFT(r._fft_in);
		  ...
		  r._disp_z_plan.DoFFT(r._fft_in);
